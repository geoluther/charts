...
# for reference
columns = ['Plays', 'AlbumTitle', 'TrackArtist', 'Artist', 'AddDate', 'AlbumId','Genre']
...
d = date.today()
outfile = 'Charts_with_label_{}.xlsx'.format(d)
...
header = ['Plays', 'Artist','Album', 'Label', 'Add Date', 'Album Id','Genre']
...
with open(infile, 'rb') as f:
	...
	for row in reader:
		...
		data = [ int(row['Plays']), artist, row['AlbumTitle'], row['Label'],
...
## just count albums.
# query = """SELECT Count(*) as Plays, AlbumTitle, TrackArtist, Artist, AddDate, AlbumId, Genre
